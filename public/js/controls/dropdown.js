...
var item = $('<div />').addClass('list-group-item half').text('Dropdown');
list.append(item);

generateItem(item, function () {
	var row = $('<div />').addClass('row form-item').css('bottom','auto').attr('data-type', 'dropdown');
	...
	var control = $('<input />')
				.attr({ type: 'dropdown', id: 'test', name: 'test', class: 'cls-value'})
	...
	if (!$form.showProperties['dropdown']){
		$form.showProperties['dropdown'] = function () {
			$('#properties').empty();
			...
			for (var i = 0; i < values.length; i++) {
				...
				if (values.length > 1) {
					...
					.click(function () {
						...
						$form.currentControl.find('.cls-value-container').attr('data-value', JSON.stringify(values));
						$form.showProperties['dropdown']();
						$form.showProperties['dropdown_generate']();
					})
					.appendTo(container);
				}
			}
...
$form.showProperties['dropdown_generate'] = function () {
