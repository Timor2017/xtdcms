...
var item = $('<div />').addClass('list-group-item half').text('Checkbox');
list.append(item);

generateItem(item, function () {
	var row = $('<div />').addClass('row form-item').css('bottom','auto').attr('data-type', 'checkbox');
	...
	var control = $('<input />')
				.attr({ type: 'checkbox', id: 'test', name: 'test', class: 'cls-value'})
	...
	if (!$form.showProperties['checkbox']){
		$form.showProperties['checkbox'] = function () {
			$('#properties').empty();
			...
			for (var i = 0; i < values.length; i++) {
				...
				if (values.length > 1) {
					...
					.click(function () {
						...
						$form.currentControl.find('.cls-value-container').attr('data-value', JSON.stringify(values));
						$form.showProperties['checkbox']();
						$form.showProperties['checkbox_generate']();
					})
					.appendTo(container);
				}
			}
...
$form.showProperties['checkbox_generate'] = function () {
